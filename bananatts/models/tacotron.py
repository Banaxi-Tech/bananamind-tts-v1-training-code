...
from .acoustic import make_padding_mask
...
class TacotronLite(nn.Module):
    def __init__(self, vocab_size: int, n_mels: int, config: dict[str, Any], pad_id: int = 0):
        super().__init__()
        ...
        self.encoder = Encoder(
            ...
            hidden,
            pad_id,
            dropout=float(config.get("encoder_dropout", 0.15)),
            conv_layers=int(config.get("encoder_conv_layers", 3)),
        )
        ...
        self.attention = LocationSensitiveAttention(
            ...
            location_channels=int(config.get("location_channels", 32)),
            location_kernel=int(config.get("location_kernel_size", 31)),
        )
        ...
        self.postnet = Postnet(
            n_mels,
            channels=int(config.get("postnet_channels", 512)),
            layers=int(config.get("postnet_layers", 5)),
            dropout=float(config.get("postnet_dropout", 0.5)),
        )
    ...
    def forward(self, tokens: torch.Tensor, token_lens: torch.Tensor, mels: torch.Tensor) -> TacotronOutput:
...
def tacotron_loss(
    ...
    guide_loss = guided_attention_loss(
        output.alignments,
        token_lens,
        mel_lens,
        reduction_factor=max(1, output.stop_logits.shape[1] // max(1, output.alignments.shape[1])),
    ...
    return loss, {
        ...
        "postnet_mel_loss": float(post_loss.detach().cpu()),
        "stop_loss": float(stop_loss.detach().cpu()),
        "guided_attention_loss": float(guide_loss.detach().cpu()),
    }
...
def guided_attention_loss(
    ...
    for batch_idx in range(alignments.shape[0]):
        ...
        # Sum over text positions first. A plain matrix mean divides by text_len
        # and makes the guide too weak to shape attention early in training.
